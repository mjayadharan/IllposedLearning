...
from Basis import OrthogonalLibrary, standardize_columns
...
def sg_smooth_and_cdiff(
    data_original: pd.DataFrame,
    window_length: int = 11,   # Must be an odd number and ≤ the number of data points.
    polyorder: int = 3,        # The degree of the polynomial must be less than the window length.
    interpolate_nan: bool = True  # whether to first perform linear interpolation on each column to handle missing values
):
    """
    Input:
        data_original: DataFrame，
        Column 1 represents time, with subsequent columns indicating status variables.
    Output:
        data_smoothed: DataFrame，Smoothed data (time + various states)
        data_deriv:    DataFrame，First-order derivative data (time + derivatives for each state, column names suffixed with _d1)
    """
    if data_original.shape[1] < 2:
        raise ValueError("The data frame must contain at least one column for time and one column for status.")

    # --- Break down time and status ---
    time_col = data_original.columns[0]
    state_cols = list(data_original.columns[1:])
    t = data_original[time_col].to_numpy(dtype=float)

    n = len(data_original)
    if n < 3:
        raise ValueError("There are too few data points (<3) to perform smoothing/differentiation.")

    # --- Correct SG parameters ---
    # window_length must be an odd number and ≤ n
    if window_length > n:
    ...
    if window_length % 2 == 0:
        window_length -= 1
    # polyorder should < window_length
    if polyorder >= window_length:
        polyorder = window_length - 1
    if polyorder < 1:
        polyorder = 1

    # --- Copy the time column to two output boxes  ---
    data_smoothed = pd.DataFrame({time_col: t})
    data_deriv    = pd.DataFrame({time_col: t})

    # --- Process each state column ---
    for c in state_cols:
        y = data_original[c].to_numpy(dtype=float)
        ...
        # 1) SG Smoothing (only when deriv=0)
        y_smooth = savgol_filter(
            y, window_length=window_length, polyorder=polyorder,
            deriv=0, mode='interp'
        )

        # 2) Second-order central finite difference (second-order at interior points, 
        # first-order at boundaries)
        # np.gradient 
        # Supports inputting uneven t values and automatically calculates differences based on coordinates.
        y_d1 = np.gradient(y_smooth, t)

        data_smoothed[c]     = y_smooth
        data_deriv[f"{c}_d1"] = y_d1

    return data_smoothed, data_deriv
...
class TimeSeriesDerivative:
    ...
    def _spline(self,t:np.ndarray,x:np.ndarray,order=1,smoothing:float = 0):
        try:
            spline = UnivariateSpline(t,x,s=None)
...
class Terms_Identification_original:
    ...
    def _strip_parameters(self, term: str) -> str:
        ...
        term = term.strip()
        # Convert "/x9" or "/x3^2"  →  "*x9^-1" or "*x3^-2"
        def _div_to_neg_pow(match):
            var_idx = match.group(1)
            pow_str = match.group(3)  # may be None
            power   = pow_str if pow_str else '1'
            return f"*x{var_idx}^-{power}"
        term = re.sub(r'/x(\d+)(\^(\d+))?', _div_to_neg_pow, term)
        term = re.sub(r'/($|\s)', ' ', term)
        term = re.sub(r'/[A-Za-z_][A-Za-z0-9_]*', '', term)
        if not term:
            return ''
...
class Terms_Identification_recovered:
    def __init__(self,model_dataframe):
        # model dataframe normally has two columns.
        # The first column represents LHS of an equation, like dx/dt.
        # The second column represents RHS of an equation, like k1*x1 + k1*k2*x3^2
    ...
    def _strip_parameters(self, term: str) -> str:
        # Remove non-state-variable symbols (parameters) from the term, leaving only xi variables.
        term = term.strip()
        # 1️. Remove parameter trailers like  '/init_SHP1' or '/kdeg'
        term = re.sub(r'/[A-Za-z_][A-Za-z0-9_]*', '', term)
        if not term:
            return ''
        
        state_vars = set()
        if hasattr(self,'variable_mapping') and self.variable_mapping:
            state_vars.update(self.variable_mapping.keys())
            state_vars.update(self.variable_mapping.values())

        found_vars = []
        # Create patterns (including powers) for each known state variable
        for var in state_vars:
            # Matching variables and their possible powers
            pattern = r'\b' + re.escape(var) + r'(?:\^(\d+))?\b'
            matches = re.finditer(pattern, term)
            
            for match in matches:
                if match.group(1):  # with power
                    found_vars.append(f"{var}^{match.group(1)}")
                else:
                    found_vars.append(var)
        
        if not found_vars:
            return ''
        # 4. Group same variables and sum their powers
        from collections import defaultdict
        var_powers = defaultdict(int)
    
        for factor in found_vars:
            if '^' in factor:
                base, power = factor.split('^')
                var_powers[base] += int(power)
            else:
                var_powers[factor] += 1
        # 5. Build result sorted by variable index
        result_parts = []
        def sort_key(var):
            ...
            return (1,var)
        for var in sorted(var_powers.keys(), key=sort_key):
            power = var_powers[var]
            if power == 1:
                result_parts.append(var)
            else:
                result_parts.append(f"{var}^{power}")
    ...
    def _extract_terms(self,equation):
        ...
        equation = equation.replace('*', ' ')
        equation = equation.replace(' ','')
        # Insert a '*' between a numeric coefficient and a following xi variable
        # e.g. '0.002x1' -> '0.002*x1'
        equation = re.sub(r'(\d(?:[\d\.eE+-]*))(?=x\d+)', r'\1*', equation)
        # Expand the equation to prevent identifying (x+y) as a term
        expanded_equation = self._expand_brackets(equation)
        terms = self._split_equation_terms(expanded_equation)
        # Remove coefficients and signs
    ...
    def _remove_coefficients(self, term: str) -> str:
        ...
        if not term :
            return ''
        var_pattern = r'x\d+(?:\^\d+)?'
        vars_found = re.findall(var_pattern, term)
        if not vars_found:
            return ''
...
class Terms_Analysis:
    def __init__(self, original_model, discovered_model, variable_mapping, data, degree=None):
        # original_model: original model in dataframe
        # recovered_model: recovered model in dataframe
        # variable_mapping: mapping for all variables in the original model
        # discovered_mapping: mapping from Recover_Model, which is a mapping between equations and discovered terms
        # data: candidate function library dataframe
    ...
    def _wrong_missing_terms(self):
        ...
        missing_terms = {}
        mixed_terms = {}
        orig_terms_xi, disc_terms_xi = {},{}
        raw_terms_disc = {}
        raw_terms_orig = {}

        orig_id = Terms_Identification_original(self.original_model)
        orig_id._parse_model()
        disc_id = Terms_Identification_recovered(self.discovered_model)
        disc_id._parse_model()
        ...
        for lhs in self.original_model.iloc[:,0].tolist():
            ...
            for t in raw_terms:
                ...
                if t_clean:
                    # convert terms like x1*x1 to x1^2
                    t_formatted = orig_id._convert_to_power_notation(t_clean)
                    clean_terms.add(t_formatted)
            orig_terms_xi[lhs_xi] = sorted(clean_terms)

         # discovered model   
        for i in range(self.discovered_model.shape[0]):
            lhs = self.discovered_model.iloc[i,0]
            rhs = self.discovered_model.iloc[i,1]
            raw_terms = disc_id._extract_terms(rhs)
            raw_terms_disc[lhs] = {
                "terms": raw_terms
            }
            clean_terms = []
            for t in raw_terms:
                t_xi = disc_id._convert_single_term_to_xi(t)
                t_clean = disc_id._strip_parameters(t_xi)
                if t_clean:
                    t_formatted = disc_id._convert_to_power_notation(t_clean)
                    clean_terms.append(t_formatted)
            disc_terms_xi[lhs] = sorted(clean_terms)
        
        ordered_keys = sorted(orig_terms_xi,key=lambda k: int(re.search(r'\d+', k).group()))
        for lhs in ordered_keys:
            orig_set = set(orig_terms_xi.get(lhs, {}))
            disc_set = set(disc_terms_xi.get(lhs, {}))

            wrong   = disc_set - orig_set
            missing = orig_set - disc_set

            wrong_terms[lhs]   = wrong
            missing_terms[lhs] = missing
            mixed_terms[lhs]   = wrong | missing

            #discovered_series = self.discovered_mapping[lhs_xi]['discovered_terms']
            #discovered_terms = set(discovered_series.index.tolist())
            #wrong = disc_terms_xi - orig_terms_xi
            #missing = orig_terms_xi - disc_terms_xi
            wrong_terms[lhs] = wrong
            missing_terms[lhs] = missing
            mixed_terms[lhs] = wrong | missing
        self.original_terms = orig_terms_xi
        self.discovered_terms = disc_terms_xi

        return wrong_terms,missing_terms,mixed_terms
    
    def _SVD_analysis(self):
        def _xi_index(var_name: str) -> int:
            m = re.match(r'x(\d+)', var_name)
            return int(m.group(1)) if m else 0
        con_number = {}
        for feature, mixed in self.Mixed_terms.items():
            # mixed can be a set OR {'Terms': set}
            term_set = mixed if isinstance(mixed, set) else mixed.get("Terms", set())

            # Canonicalize terms: sort variables by their numeric index, not lexicographically
            canon_terms = []
            for t in term_set:
            ...
            if data.shape[1] == 0 or data.shape[0] == 0:
                deg_info = f"(degree={self.degree})" if getattr(self, "degree", None) is not None else "(degree=unknown)"
                print(f"{feature} {deg_info}: no wrong terms or missing terms, skipping SVD.")
