...
class Recover_Model_sindy:
    def __init__(self,data,differential_order,poly_degree,threshold):
        ...
        self.model_expression = self._recovered_model()
...
class Terms_Identification_recovered:
    ...
    def _strip_parameters(self, term: str) -> str:
        ...
        for f in factors:
            for xi in xi_vars:
                for var in re.findall(r'x\d+\^?\d*', f):
                    xi_factors.append(var)
                #if f.startswith(xi):
                    #xi_factors.append(f)
                    #break
        return '*'.join(sorted(xi_factors)) if xi_factors else ''
...
class Terms_Analysis:
    def __init__(self,original_model,discovered_model,variable_mapping,data):
